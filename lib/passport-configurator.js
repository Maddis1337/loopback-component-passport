...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var AuthStrategy = require(options.module);  // allows for strategies to be at the root (token based strategies break this convention)
  if(!AuthStrategy || typeof AuthStrategy !== 'function') {
    AuthStrategy = AuthStrategy[options.strategy || 'Strategy'];
  }
  var tokenBased = !!options.tokenBased;
   ...
   */
  if (authType === 'local') {
    self.app.post(authPath, options.customCallback || defaultCallback);
    ...
    var method = tokenBased? 'post': 'get';
    self.app[method](authPath, passport.authenticate(name, _.defaults({ // allow post and get
      scope: scope,
      session: session
    }, options.authOptions)));

    if(tokenBased) { // token based login will not use callback in traditional sense, but loads user info in the same request
        self.app.post(authPath, options.customCallback || defaultCallback);
        self.app.get(authPath, options.customCallback || defaultCallback); // also allow via get parameters
    }
  }
   ...
   */
  if (!tokenBased && link) { // do not register callbacks for token based strategies
